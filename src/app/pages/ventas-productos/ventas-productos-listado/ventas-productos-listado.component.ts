...
@Component({
  ...
  templateUrl: './ventas-productos-listado.component.html',
  styleUrl: './ventas-productos-listado.component.scss',
})
export class VentasProductosListadoComponent {
  ...
  eliminar(venta: Venta) {
    Swal.mixin({
      ...
      .fire({
        title: '¿Estás seguro?',
        text: 'Esta venta se eliminará y no podrás recuperarla.',
        icon: 'warning',
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          this._VentaService
            .deleteVenta(Number(venta.id))
            .pipe(takeUntil(this.destruir$))
            .subscribe((data) => {
              this.VentaList.data = this.VentaList.data.filter(
                (gast) => gast.id != venta.id
              );
